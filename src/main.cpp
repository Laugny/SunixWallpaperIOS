...
=======
        // … dein bestehender Code für gInLevelOrEditor + replaceBG …
>>>>>>> e41d371 (initial)
...
=======
                } else if (gEmitter && gEmitter->getParent()) {
                    gEmitter->removeFromParentAndCleanup(false);
>>>>>>> e41d371 (initial)
                }
            }
...
=======
    ...
    // ✨ NEU: wird aufgerufen, wenn Fenster/Viewport/Projection sich ändert (z.B. Fullscreen ⇄ Windowed)
    void reshapeProjection(const CCSize& newSize) {
        CCDirector::reshapeProjection(newSize); // Basis macht Viewport/Projection

        // Emitter „wieder andocken“ & auf neue Breite anpassen – ohne Reset!
        if (gEmitter) {
            // Emissionszone an neue Breite anpassen
            ...
            // Falls wir im Menü sind: sicherstellen, dass der Emitter wieder am BG hängt
            if (!gInLevelOrEditor) {
                if (auto scene = this->getRunningScene()) {
                    if (auto layer = getChildOfType<CCLayer>(scene, 0)) {
                        ensureBG(layer, -999);          // falls BG durch Rebuild fehlt
                        attachEmitterToMenuBG(layer);   // reparent ohne Cleanup
                    }
                }
            }

            // Für den Fall, dass Scheduler/Actions pausiert wurden
